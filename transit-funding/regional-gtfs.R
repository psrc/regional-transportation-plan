...
# Agency Trip Summary -----------------------------------------------------
agency.summary <- route.hours.of.service %>%
  select(agency, transit_mode, daily_trips) %>%
  mutate(route=1) %>%
  group_by(agency,transit_mode) %>%
  summarize(total_trips=sum(daily_trips), total_routes=sum(route)) %>%
  mutate(trips_per_route=round(total_trips/total_routes,1)) %>%
  select(agency,transit_mode,total_routes,total_trips,trips_per_route)

write_csv(agency.summary, "output/agency_summary.csv")
write_csv(route.hours.of.service, "output/route_summary.csv")
